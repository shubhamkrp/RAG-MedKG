...
def calculate_f1(true_codes, predicted_codes):
    ...
    precision = true_positives / len(true_codes) #as the len(true_label)==len(predicted_label)
...
import csv
import pandas as pd
raw_test_df = pd.read_csv('input.csv')
raw_test_df = raw_test_df.drop('Unnamed: 0',axis =1)
...
with open(output_file_path, mode='w', newline='', encoding="utf-8") as outfile:
    ...
    for i, j in tqdm(raw_test_df.iterrows(), total=len(raw_test_df)):
        true_label = j.SHORT_CODES.split(',')
        true_length = len(true_label)
        pred_label = get_predictions(str(j.text) + ' ' + str(j.Symptoms),true_length,0.0643745388060965)
        true_labels.append(true_label)
        predicted_labels.append(pred_label)
